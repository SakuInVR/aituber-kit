...
const Cockpit: React.FC = () => {
  ...
  const [apiEndpoint, setApiEndpoint] = useState('http://localhost:3000/api/messages');
  ...
  // 編集保存
  const handleSave = () => {
    const lines = editLines.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
    if (!editName.trim() || lines.length === 0) return;
    if (editScript) {
      setScripts(scripts.map(s => s.id === editScript.id ? { ...s, name: editName, lines } : s));
  ...
  const conversationContinuityMode = settingsStore((s) => s.conversationContinuityMode);

  // YouTubeコメント取得
  const [youtubeApiKey, setYoutubeApiKey] = useState(process.env.NEXT_PUBLIC_YOUTUBE_API_KEY || '');
  const [youtubeLiveId, setYoutubeLiveId] = useState(process.env.NEXT_PUBLIC_YOUTUBE_LIVE_ID || '');
  const [ytComments, setYtComments] = useState<{id:string, author:string, message:string, publishedAt:string}[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      setYtLoading(true);
      try {
        const res = await fetch(`/api/youtube-comments?apiKey=${encodeURIComponent(youtubeApiKey)}&liveId=${encodeURIComponent(youtubeLiveId)}`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-5xl mx-auto bg-white rounded-lg shadow-lg p-6">
        <h1 className="text-3xl font-bold mb-6 text-center">AITuberコックピット</h1>
        {/* モード切り替えスイッチ */}
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <div className="flex-1 flex items-center gap-2">
            ...
            <button
              ...
              onClick={() => settingsStore.setState({ youtubeMode: !youtubeMode })}
            >
              {youtubeMode ? 'ON' : 'OFF'}
            </button>
          </div>
          <div className="flex-1 flex items-center gap-2">
            ...
            <button
              ...
              onClick={() => settingsStore.setState({ conversationContinuityMode: !conversationContinuityMode })}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <section className="bg-blue-50 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600 mb-2">よく使うセリフを即座に発話</p>
          ...
          <section className="bg-green-50 rounded-lg p-4">
            ...
            <div className="flex gap-2 mb-2">
              ...
              <button className="bg-blue-500 text-white rounded px-3" onClick={handleAdd}>＋</button>
              {scripts.length > 1 && (
                <button className="bg-red-500 text-white rounded px-3" onClick={() => handleDelete(selectedScriptId)}>削除</button>
              )}
              <button className="bg-gray-500 text-white rounded px-3" onClick={() => handleEdit(scripts.find(s => s.id === selectedScriptId))}>編集</button>
            </div>
            <ul className="mb-2 bg-white border rounded p-2">
              {scripts.find(s => s.id === selectedScriptId)?.lines.map((line, idx) => (
                <li key={idx} className="mb-1 text-gray-800">{line}</li>
              ))}
            </ul>
            <button
            ...
            {isEditing && (
              <div className="mt-4 p-4 bg-gray-100 rounded">
                <div className="mb-2 font-bold">{editScript ? '台本を編集' : '新規台本を追加'}</div>
                <input
                ...
                <div className="flex gap-2 justify-end">
                  <button className="bg-gray-400 text-white rounded px-3" onClick={handleCancel}>キャンセル</button>
                  <button className="bg-blue-500 text-white rounded px-3" onClick={handleSave}>保存</button>
                </div>
              </div>
            )}
          </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          ...
          <section className="bg-yellow-50 rounded-lg p-4">
            <h2 className="text-xl font-bold mb-2">インテリジェント・コメントピッカー</h2>
            <p className="text-sm text-gray-600 mb-2">視聴者コメントを選んでAIに渡す</p>
            ...
            <div className="mb-2 text-xs text-gray-500">YouTubeコメント（最新20件）</div>
            <ul className="mb-2 max-h-40 overflow-y-auto bg-white border rounded p-2">
              ...
              {ytComments.map((c) => (
                <li
                  ...
                  title={c.author}
                >
                  <span className="text-xs text-gray-500 mr-2">{c.author}:</span>{c.message}
                </li>
              ))}
              {ytComments.length === 0 && !ytLoading && <li className="text-gray-400">コメントがありません</li>}
            </ul>
            <div className="mb-2 text-xs text-gray-500">（手動追加コメント）</div>
            <ul className="mb-2">
          ...
          <section className="bg-purple-50 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600 mb-2">AIへの自由指示を即時送信</p>
